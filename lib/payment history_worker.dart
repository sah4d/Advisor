...
class payhisW extends StatefulWidget {
  const payhisW({super.key});

  @override
  State<payhisW> createState() => _payhisWState();
}

class _payhisWState extends State<payhisW> {
  _payhisWState() {
    prefs();
    fetchAdsData();
  }
  ...
  Future<void> fetchAdsData() async {
    ...
    QuerySnapshot querySnapshot = await adsRef.where("paid to", isEqualTo: pref.getString("phonenum").toString()).get();
