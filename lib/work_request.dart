...
class RequestWork extends StatefulWidget {
  @override
  State<RequestWork> createState() => _RequestWorkState();
}

class _RequestWorkState extends State<RequestWork> {
  String id_ = "";
  String namel_ = '';
  String place_ = '';
  String price_ = "";
  ...
  Future<void> shared() async {
    ...
    setState(() {
      ...
      workername = prefs.getString("workername").toString();
      PAy = prefs.getString("payment").toString();
    });
  }

  Future<void> UpdateAd() async {
    DocumentReference firedb = FirebaseFirestore.instance.collection("advertisement").doc(id_);
    CollectionReference firedk=FirebaseFirestore.instance.collection("requests");
    firedk.doc().set({'ad':firedb,"worker ph":phonenum,"worker name":workername,"selected":"Not Selected", "Date":DateFormat.yMMMEd().format(DateTime.now()).toString()});
  }
      ...
      Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            _buildDetailRow('Location', place_),
            ...
            ElevatedButton(
              onPressed: () {
                ...
                Alert(
                  context: context,
                  title: "Request Sent",style: AlertStyle(isButtonVisible: false,
                    titleStyle: GoogleFonts.dmSans(fontSize: 15)),
                  image: Image.asset("assets/request.png",height: 70,),
                ).show();
              },
              child: Text(
                'Request for Work',
