                            ...
                            ElevatedButton(
                                onPressed: () async {
                                  if (_formkey.currentState!.validate()) {
                                    ...
                                    CollectionReference firedj = FirebaseFirestore.instance.collection("logindetailsworker");
                                    CollectionReference firedk = FirebaseFirestore.instance.collection("logindetailsuser");
                                    QuerySnapshot qs2=await firedk.where("phone",isEqualTo: _phoneContoller.text).get();
                                    QuerySnapshot querysnapshot= await firedj.where("phone",isEqualTo: _phoneContoller.text).get();
                                    final _doc2=querysnapshot.docs;
                                    final _doc0=qs2.docs;
                                    if(_doc2.length==0 && _doc0.length==0){
                                      SharedPreferences prefs=await SharedPreferences.getInstance();
                                      firedj.add({"phone":_phoneContoller.text,"password":_PassController.text,"type":"Worker"});
                                      firedb.add({"name":_nameController.text,"place":_placeController.text,"qualification":_qualificationController.text,"phone":_phoneContoller.text,"password":_PassController.text,"photo":prefs.getString('photo').toString(),});
