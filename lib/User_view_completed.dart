...
class BookedAdsU extends StatefulWidget {
  const BookedAdsU({super.key});

  @override
  State<BookedAdsU> createState() => _BookedAdsUState();
}

class _BookedAdsUState extends State<BookedAdsU> {
  _BookedAdsUState() {
    prefs();
    fetchAdsData();
  }
  ...
  Future<void> fetchAdsData() async {
    ...
    QuerySnapshot querySnapshot = await adsRef.where("customer phone", isEqualTo: pref.getString("lid11").toString()).get();
